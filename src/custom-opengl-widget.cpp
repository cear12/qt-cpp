...
void CustomOpenGLWidget::setupVertexAttribs() {
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, kStride, reinterpret_cast<void*>(3 * sizeof(GLfloat)));
    m_vbo.release();
}
...
void CustomOpenGLWidget::paintGL() {
    ...
    m_rotationAngle += m_angularSpeed + 0.4;  // small constant spin plus any drag-imparted speed
...
void CustomOpenGLWidget::resizeGL(int width, int height) {
    ...
    qreal aspect = height != 0 ? static_cast<qreal>(width) / static_cast<qreal>(height) : 1.0;
