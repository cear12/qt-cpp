...
StarDelegate::StarDelegate(QWidget* parent) : QStyledItemDelegate(parent) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        starPolygon << QPointF(0.5 + radius * std::cos(angle), 0.5 + radius * std::sin(angle));
    }

    // A simple diamond, used to mark a rating of zero.
    diamondPolygon << QPointF(0.5, 0.0) << QPointF(1.0, 0.5) << QPointF(0.5, 1.0) << QPointF(0.0, 0.5);
}
...
void StarDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    painter->setBrush(option.state & QStyle::State_Selected ? option.palette.highlightedText() : QBrush(Qt::darkYellow));

    if (rating == 0) {
        painter->translate(option.rect.x(), option.rect.y() + (option.rect.height() - kStarSize) / 2);
...
QWidget* StarDelegate::createEditor(QWidget* parent, const QStyleOptionViewItem& /*option*/,
    ...
    connect(editor, &QSpinBox::editingFinished, this, &StarDelegate::commitAndCloseEditor);
    return editor;
}
