...
void runStrategyDemo() {
    ...
    QPixmap custom = processor.applyCustomFilter([](const QPixmap& in) { return in; });

    bool ok = filters.size() == 2 && !blurred.isNull() && custom.size() == image.size();
    std::cout << "  [" << (ok ? "PASS" : "FAIL") << "] filters registered and applied without error\n";
}
...
void runCompositeDemo() {
    ...
    std::cout << "  Root panel child count: " << root->childCount() << " (expected 2: title label + button row)\n";
    std::cout << "  Root sizeHint: " << root->sizeHint().width() << "x" << root->sizeHint().height() << "\n";
    ...
    std::cout << "  [" << (clicked ? "PASS" : "FAIL") << "] button click handler fired\n";
}
...
void runModelViewDelegateDemo() {
    ...
    std::cout << "  rowCount=" << model.rowCount() << " columnCount=" << model.columnCount() << "\n";
    std::cout << "  cell(0, Name)=" << model.data(model.index(0, PersonModel::NameColumn)).toString().toStdString()
               << "\n";

    bool ok = model.rowCount() == 3 && model.columnCount() == PersonModel::ColumnCount;
    std::cout << "  [" << (ok ? "PASS" : "FAIL") << "] model dimensions correct\n";
}
...
void runStarDelegateDemo() {
    ...
    std::cout << "  StarDelegate sizeHint: " << delegate->sizeHint(QStyleOptionViewItem(), QModelIndex()).width()
               << "x" << delegate->sizeHint(QStyleOptionViewItem(), QModelIndex()).height() << "\n";
    std::cout << "  [PASS] StarDelegate constructed and attached to a QTableView\n";
}
...
void runPluginWidgetDemo() {
    ...
    std::cout << "  isInitialized: " << (plugin.isInitialized() ? "true" : "false") << "\n";

    std::unique_ptr<QWidget> widget(plugin.createWidget(nullptr));
    auto* custom = qobject_cast<CustomWidget*>(widget.get());
    bool ok = custom != nullptr && custom->caption() == QStringLiteral("Custom Widget");
    std::cout << "  [" << (ok ? "PASS" : "FAIL") << "] createWidget() returns a real, working CustomWidget\n";
}
...
void runOpenGlWidgetDemo() {
    ...
    Q_UNUSED(widget)
    std::cout << "  [PASS] CustomOpenGLWidget constructed OK (not shown -- see README)\n";
}
...
void runAnimatedGraphicsDemo() {
    ...
    std::cout << "  boundingRect: " << item->boundingRect().width() << "x" << item->boundingRect().height() << "\n";
    std::cout << "  [PASS] item constructed and added to a QGraphicsScene\n";
}

void runWidgetCreationDemo(QApplication& app) {
    std::cout << "\n=== Cross-thread widget creation (multi-threading-widget-creation.h) ===\n";

    WidgetCreator creator;
    WorkerThread worker;
    bool created = false;

    QObject::connect(&worker, &WorkerThread::requestWidgetCreation, &creator, &WidgetCreator::createWidgetSafely);
    QObject::connect(&creator, &WidgetCreator::widgetCreated, &app, [&created]() { created = true; });
    ...
    std::cout << "  [" << (created ? "PASS" : "FAIL") << "] widget created safely on the GUI thread\n";
}
