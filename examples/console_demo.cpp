...
void runFactoryDemo() {
    ...
    std::cout << "  Circle area:    " << (circle ? circle->area() : -1.0) << " (expected ~78.54)\n";
    std::cout << "  Rectangle area: " << (rect ? rect->area() : -1.0) << " (expected 200)\n";
    std::cout << "  Unregistered type returns null: " << (missing == nullptr ? "yes" : "NO (bug)") << "\n";

    bool circleOk = circle && std::abs(circle->area() - 78.53981633974483) < 1e-9;
    bool rectOk = rect && std::abs(rect->area() - 200.0) < 1e-9;
    std::cout << "  [" << ((circleOk && rectOk && !missing) ? "PASS" : "FAIL") << "] factory results correct\n";
}
...
void runUndoRedoDemo() {
    ...
    undoStack.undo();
    undoStack.undo();
    std::cout << "  After two more undos: \"" << doc.content().toStdString() << "\" (expected empty)\n";
    ...
    std::cout << "  [" << (ok ? "PASS" : "FAIL") << "] undo/redo round-trip correct\n";
}
...
void runPropertyWrapperDemo() {
    ...
    std::cout << "  [" << (label == QStringLiteral("demo-source") ? "PASS" : "FAIL") << "] property get/set correct\n";

    bool signalFired = false;
    wrapper.connectToSignal([&signalFired]() { signalFired = true; });
    emit source.dataChanged(1);  // manually fire, rather than waiting a full second for the timer
    std::cout << "  [" << (signalFired ? "PASS" : "FAIL") << "] connectToSignal wiring fired\n";
}
...
void runAsyncProcessorDemo() {
    ...
    QObject::connect(&processor, &AsyncProcessor::operationCompleted, [&](const QString& result) {
        completed = true;
        resultText = result;
    });

    processor.watchFuture(processor.processDataAsync(QByteArray("some demo payload bytes")));

    QEventLoop loop;
    QObject::connect(&processor, &AsyncProcessor::operationCompleted, &loop, &QEventLoop::quit);
    QTimer::singleShot(4000, &loop, &QEventLoop::quit);  // safety timeout
    loop.exec();

    std::cout << "  result: " << resultText.toStdString() << "\n";
    std::cout << "  [" << (completed ? "PASS" : "FAIL") << "] async operation completed and was watched correctly\n";
}

void runNetworkConstructionCheck() {
    std::cout << "\n=== Network classes (construction only, no real I/O -- see README) ===\n";
    AdvancedNetworkManager networkManager;
    NetworkDataService realService;
    Q_UNUSED(networkManager)
    std::cout << "  AdvancedNetworkManager, NetworkDataService constructed OK (not exercised: no live server)\n";
...
void runBackEndDemo() {
    ...
    std::cout << "  out-of-range getData(5) is invalid: " << (!backend.getData(5).isValid() ? "yes" : "NO (bug)")
               << "\n";

    registerQmlTypes();  // the line that used to be a bare, non-compiling statement in the header
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "\n=== State machine + Observer (state-machine-controller.h, signals-slots-observer.h) ===\n";
    std::cout << "  (runs on the event loop for a few seconds)\n";

    StateMachineController controller;
    QObject::connect(&controller, &StateMachineController::statusChanged, [](const QString& status) {
    ...
    QObject::connect(&source, &DataSource::dataChanged, &observer1, &Observer::onDataChanged);
    QObject::connect(&source, &DataSource::milestone, &observer2, &Observer::onMilestone);
