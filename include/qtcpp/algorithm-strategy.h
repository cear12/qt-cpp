...
// Strategy pattern: interchangeable image-filter algorithms applied to a
// QPixmap by a context object (ImageProcessor) that doesn't know which
// concrete filter it's using.
#include <QColor>
#include <QObject>
#include <QPainter>
#include <QPixmap>
...
class BlurFilter : public IImageFilter {
 ...
 public:
  ...
  QPixmap apply(const QPixmap& input) override {
    // Simplified stand-in for a real blur: a real implementation would
    // run a separable Gaussian/box kernel over the pixel data. This
    // demonstrates the Strategy pattern's shape, not a production blur.
  ...
  QString name() const override {
    return QString("Blur (radius: %1)").arg(m_radius);
  }
};
