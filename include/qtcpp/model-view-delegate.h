...
// A small custom QAbstractTableModel plus a QStyledItemDelegate that adds
// extra painting for one column -- the standard Model/View/Delegate pattern.
...
public:
    enum Column { NameColumn = 0, AgeColumn, EmailColumn, ColumnCount };

    explicit PersonModel(QObject* parent = nullptr) : QAbstractTableModel(parent) {
        m_persons = {{"Alice", 25, "alice@example.com"},
    ...
    QVariant data(const QModelIndex& index, int role = Qt::DisplayRole) const override {
        if (!index.isValid() || index.row() >= static_cast<int>(m_persons.size())) return QVariant();

        const Person& person = m_persons[static_cast<std::size_t>(index.row())];
    ...
    bool setData(const QModelIndex& index, const QVariant& value, int role) override {
        if (role == Qt::EditRole && index.isValid()) {
            Person& person = m_persons[static_cast<std::size_t>(index.row())];
            switch (index.column()) {
                case NameColumn:
...
public:
    ...
    void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const override {
        ...
        // Highlight anyone under 30 in the age column.
