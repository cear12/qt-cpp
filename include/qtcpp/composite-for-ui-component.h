...
// Composite pattern over Qt widgets: LabelComponent/ButtonComponent are
// leaves, PanelComponent is a composite that owns child IUIComponents and a
// real QBoxLayout, and UIBuilder is a small fluent builder on top for
// assembling a tree of panels without manual new/addWidget bookkeeping.
#include <QBoxLayout>
#include <QHBoxLayout>
#include <QLabel>
#include <QPushButton>
...
class ButtonComponent : public QObject, public IUIComponent {
 ...
 public:
  explicit ButtonComponent(const QString& text,
                           std::function<void()> handler = nullptr)
      : m_button(std::make_unique<QPushButton>(text)), m_clickHandler(handler) {
    if (m_clickHandler) {
      connect(m_button.get(), &QPushButton::clicked, this, m_clickHandler);
    }
  }
...
class PanelComponent : public IUIComponent {
 ...
 public:
  ...
  T* createAndAdd(Args&&... args) {
    static_assert(std::is_base_of_v<IUIComponent, T>,
                  "T must implement IUIComponent");
...
// Fluent builder for assembling a tree of panels.
class UIBuilder {
 ...
 public:
  ...
  UIBuilder& addButton(const QString& text,
                       std::function<void()> handler = nullptr) {
    m_panelStack.top()->createAndAdd<ButtonComponent>(text, handler);
    return *this;
  }

  UIBuilder& beginPanel(
      PanelComponent::LayoutType type = PanelComponent::Vertical) {
  ...
  std::unique_ptr<PanelComponent> build() { return std::move(m_root); }
};
